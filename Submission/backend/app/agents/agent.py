...
class Agent:
    ...
    def act(self, step_count: int, world, external_action: Optional[int] = None):
        """
        The Main Execution Cycle.
        If `external_action` is provided (from RL), it bypasses the Internal Brain/Qalb proposal.
        """
        ...
        # B. RL EXTERNAL OVERRIDE (For Training/Inference)
        if not action_plan and external_action is not None:
             # Map Discrete(7) to Action Plan
             # 0: Wait, 1: Up, 2: Down, 3: Left, 4: Right, 5: Interact
             mapping = {
                 0: {'action': 'wait', 'desc': 'RL Wait'},
                 1: {'action': 'step_y', 'val': -1, 'desc': 'RL Up'},
                 2: {'action': 'step_y', 'val': 1, 'desc': 'RL Down'},
                 3: {'action': 'step_x', 'val': -1, 'desc': 'RL Left'},
                 4: {'action': 'step_x', 'val': 1, 'desc': 'RL Right'},
                 5: {'action': 'interact_generic', 'desc': 'RL Interact'},
                 6: {'action': 'craft_generic', 'desc': 'RL Craft'}
             }
             action_plan = mapping.get(external_action)
             if action_plan:
                 # self.log_diary(f"RL: Action {external_action}")
                 pass

        # C. Brain Planning (Long-term) [Legacy/Fallback]
        if not action_plan:
    ...
    def execute_action(self, plan: Dict, world):
        ...
        if action == 'wait':
            ...
            dx = plan.get('val', 0)
            self.move_by(dx, 0, world)
            
        elif action == 'step_y':
            dy = plan.get('val', 0)
            self.move_by(0, dy, world)

        elif action == 'interact_generic':
            # Phase 1: Eat if food is here
            # Phase 2: Gather if resource is here
            # Phase 3: Socialize if agent is here
            self.gather(world) # Covers eating and resource gathering
            # self.socialize_here(world) # Future
            
        elif action == 'craft_generic':
            # Phase 4: Civilization Integration
            # Contextual "Smart" Action:
            # 1. If I have Stone Block -> Build Wall
            # 2. If I have Stone -> Craft Block
            
            blocks = [s for s in self.inventory if s['item']['name'] == 'Stone Block']
            ...
            if blocks:
                ...
                blocks[0]['count'] -= 1
                if blocks[0]['count'] <= 0:
                    self.inventory.remove(blocks[0])
                
                # Create Wall Item
                from ..env.item import Item
                import uuid
                wall = Item(id=f"wall_{uuid.uuid4()}", name="Wall", weight=100.0, hardness=1.0, durability=10.0, tags=["building", "defense"])
                
                # Place in World
                world._add_item(self.x, self.y, wall)
                self.log_diary("Built a Wall.")
                ...
                self.log_diary("RlCraft Failed: No Materials.")
    ...
    def move_by(self, dx: int, dy: int, world):
        """
        Executes a relative move. Used by RL and simple heuristics.
        """
        success = world.move_agent(self.id, dx, dy)
        if success:
            self.nafs.energy = max(0, self.nafs.energy - 0.005) # Small cost
            # Update last known position handled by world, but we could update internal tracking if any
        return success
    ...
    def die(self, world, cause="unknown"):
        ...
        """
        world.log_event(f"DEATH: {self.attributes.name} has died of {cause}. (Age: {self.state.age_steps})")
