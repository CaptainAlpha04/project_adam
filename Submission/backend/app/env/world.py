...
class World:
    ...
    def respawn_resources(self):
        """Respawn resources based on time step."""
        # Fruits: Every 100 steps
        if self.time_step % 1000 == 0:
            self._spawn_random_items("Fruit", 0.02, [2], ["food", "consumable", "red"])
        
        # Trees (Wood): Every 2000 steps
        if self.time_step % 60000 == 0:
            self._spawn_random_items("Wood", 0.15, [3], ["flammable", "material"])

        # Rocks (Stone): Every 6000 steps
        if self.time_step % 120000 == 0:
    ...
    def _get_agent_at(self, x: int, y: int) -> Optional[str]:
        # This is slow O(N), optimize if needed with a spatial map
    ...
    def log_event(self, message: str):
        ...
        if not hasattr(self, 'logs'):
            self.logs = []
        self.logs.append(f"[Step {self.time_step}] {message}")
        if len(self.logs) > 50:
            self.logs.pop(0)
